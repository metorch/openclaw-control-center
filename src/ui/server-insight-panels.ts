...
function createInsightRenderers(deps) {
  ...
  function connectorInsightStatus(snapshot, language) {
    ...
    let detail = pickUiText(language, "All six key usage sources are connected, so usage, quota, and attribution can be trusted.", "6 项关键用量数据都已接通，用量、额度和归因都可以直接看。");
    ...
    if (connectedCount === 0 && partialCount === 0) {
      detail = pickUiText(language, "None of the six key usage sources are connected yet, so the Usage page will stay very limited until you connect them in Settings.", "6 项关键用量数据都还没接通，所以用量页只会显示很有限的信息。先去设置页按建议接入即可。");
    ...
    return { status: "warn", connectedCount, totalCount, missingCount, detail, value };
  }

  function localizeConnectionInsightValue(item, language) {
    if (item.value === "loading") return pickUiText(language, "Loading", "读取中");
    if (item.key === "gateway") return item.status === "ok" ? pickUiText(language, "Connected", "已接通") : pickUiText(language, "Unavailable", "未接通");
    if (item.key === "config") return item.status === "ok" ? pickUiText(language, "Ready", "已就绪") : pickUiText(language, "Needs fix", "待修复");
    return item.value;
  }
  ...
  function localizeConnectionInsightDetail(item, language) {
    if (item.key === "gateway") {
      ...
      return item.status === "ok" ? item.detail : pickUiText(language, "Gateway is not reachable.", "当前还无法连到 Gateway。");
    }
    ...
    if (configuredMatch) {
      ...
      return pickUiText(language, `${agentCount} agent${agentCount === 1 ? "" : "s"} configured, but no recent sessions yet`, `已配置 ${agentCount} 个智能体，但还没有最近会话`);
    }
  ...
  function renderOpenClawConnectionCard(summary, usageCost, language) {
    if (!summary) {
    ...
    <div class="overview-command-head">
      ...
      <a class="btn" href="${escapeHtml(buildHomeHref({ quick: "all" }, true, "settings", language))}">${escapeHtml(pickUiText(language, "Open settings", "查看设置"))}</a>
    </div>
  ...
  function renderSettingsEnvironmentStatusCard(connectionSummary, usageCost, securitySummary, updateSummary, language) {
  ...
  function renderSettingsConfigAccessCard(importGuardRows, usageConnectorTodos, settingsBudgetLimitCard, language) {
    ...
    <div class="settings-status-grid settings-config-grid">
  ...
  function renderSettingsBudgetLimitCard(model, language, variant = "standalone") {
    ...
    const fileState = model.loadedFromFile ? t("Saved to runtime file", "已写入运行时文件") : t("Using starter template until first save", "当前还是启动模板，首次保存后会落盘");
    const aliasState = t("This writes agent.main.cost. Primary-controller aliases are recognized automatically.", "这里会写入 agent.main.cost，主控别名会自动识别，不需要再配第二份。");
    const issueText = model.issues.length > 0 ? model.issues.join(" | ") : fileState;
    ...
    <div class="overview-command-head">
      <div>
        <h2>${escapeHtml(t("Budget limit", "预算限额"))}</h2>
        <div class="meta">${escapeHtml(t("Just enter one number here. This is the main agent cost limit used by the usage warning card.", "这里只要填一个数字即可。这是用量预警卡使用的主智能体 cost 限额。"))}</div>
      </div>
      <div>${badge(configured ? "ok" : "info", configured ? t("Configured", "已配置") : t("Not set yet", "尚未设置"))}</div>
    </div>
    <div class="status-strip compact">
      <div class="status-chip"><span>${escapeHtml(t("Current limit", "当前限额"))}</span><strong data-budget-limit-current>${escapeHtml(configured ? model.currentLimit.toFixed(2) : t("Not set", "未设置"))}</strong></div>
    ...
    <div class="settings-budget-form">
      <label class="settings-budget-field">
        <span>${escapeHtml(t("Budget limit value", "预算限额数值"))}</span>
        <input
      ...
      <div class="settings-budget-actions">
        <button class="btn primary" type="button" data-budget-limit-save>${escapeHtml(t("Save limit", "保存限额"))}</button>
        <button class="btn" type="button" data-budget-limit-clear>${escapeHtml(t("Clear", "清空"))}</button>
      </div>
    </div>
    <div class="meta">${escapeHtml(aliasState)}</div>
    <div class="meta">${escapeHtml(issueText)}</div>
    <div class="meta" data-budget-limit-status>${escapeHtml(t("Saving here creates or updates runtime/budgets.json directly.", "这里保存后会直接创建或更新 runtime/budgets.json。"))}</div>
  </section>`;
  }
  ...
  function renderContextPressureCard(snapshot, language) {
    ...
    <div class="overview-command-head">
      ...
      <div>${badge(criticalCount > 0 ? "warn" : warnCount > 0 ? "info" : "ok", criticalCount > 0 ? pickUiText(language, "High", "较高") : warnCount > 0 ? pickUiText(language, "Watch", "观察中") : pickUiText(language, "Healthy", "正常"))}</div>
    </div>
    <div class="status-strip">
    ...
    <div class="decision-list">${ranked.slice(0, 6).map((item) => {
      ...
      return `<a class="decision-row" href="${escapeHtml(href)}">
          <div class="decision-row-copy">
            ...
            <div class="meta">${badge(tone, item.thresholdState === "critical" ? pickUiText(language, "Critical", "临界") : item.thresholdState === "warn" ? pickUiText(language, "Warning", "预警") : item.thresholdState === "ok" ? pickUiText(language, "Healthy", "正常") : pickUiText(language, "Unknown", "未知"))} ${escapeHtml(humanizeOperatorLabel(item.agentId))} · ${escapeHtml(item.model)}</div>
