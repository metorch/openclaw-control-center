...
test("task board source includes delete controls and card/details view switch", async () => {
  ...
  assert(taskBoardScriptSource.includes("if (result && typeof result.then === 'function') {"));
  assert(taskBoardScriptSource.includes("window.location.href = '/?section=collaboration&roomId=' + encodeURIComponent(roomId);"));
  assert(taskBoardScriptSource.includes("requestCollaborationRoomOpen(roomId);"));
  assert(taskBoardScriptSource.includes("buttons.forEach((button) => {"));
  assert(taskBoardScriptSource.includes("button.addEventListener('click', () => {"));
  assert(taskBoardScriptSource.includes("panel.setAttribute('aria-hidden', active ? 'false' : 'true');"));
  ...
  assert(collaborationBootSource.includes("window.__openclawOpenCollaborationRoom = (roomId, source) => openRoomFromExternalTrigger(roomId, source);"));
  ...
  assert(serverSource.includes('id="task-queue-tab-followup"'));
  assert(serverSource.includes('id="task-queue-panel-followup"'));
